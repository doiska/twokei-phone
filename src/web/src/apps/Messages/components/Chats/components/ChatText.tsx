...
const ChatText: React.FC<{ message: Message; position: string }> = ({ message: { message, author, date }, position }) => {
	const dateToText = dayjs(date).format('HH:mm');

	return (
		<li className={`flex min-w-[25%] max-w-[70%] flex-col flex-wrap gap-2 rounded-md p-2 pl-3 text-black ${style(position)}`}>
