...
export const NotificationProvider: React.FC = ({ children }) => {
    ...
    const hasAppNotification = useCallback((appId: string): number => {
        return notifications.findIndex((n) => n.app === appId);
    }, [notifications]);

    const hasNotification = useCallback((id: string): INotification | null => {
        return notifications.find((n) => n.id === id) || null;
    }, [notifications]);

    const _showAlert = useCallback((n: INotification, cb: (n: INotification) => void, sound?: string) => {
