...
const SettingsItemRange: React.FC<Props> = ({ label, icon, value, onCommit }) => {
    ...
    return (
        <SettingsItemBody icon={icon}>
            ...
            <SettingsItemOption>
                <input type="range" min={0} max={100} className='toggle-primary' onChange={(e) => setRange(e.target.value)} onMouseUp={(e,) => onCommit(e, range)} />
            </SettingsItemOption>
        </SettingsItemBody>
    )
}
...
const SettingsItemToggle: React.FC<Props> = ({ label, icon, value, onCommit }) => {
    return (
        <SettingsItemBody icon={icon}>
            ...
            <SettingsItemOption>
                <input className='toggle self-end' type="checkbox" checked={(value === 'true' || value === true)} onChange={(e) => onCommit(e, e.target.checked)} />
            </SettingsItemOption>
        </SettingsItemBody>
    )
}
...
const SettingsItemSelect: React.FC<PropsWithOptions> = ({ label, icon, value, onCommit, options }) => {
    ...
    return (
        <SettingsItemBody icon={icon}>
            <SettingsItemLabel label={label} />
            <SettingsItemOption>
                <select className="select select-bordered w-full max-w-xs" value={value as string} onChange={(e) => onCommit(e, e.target.value)}>
                    {options.map(({ label: optLabel, value: optValue }) => <option value={optValue} key={optLabel} >{optLabel}</option>)}
                </select>
            </SettingsItemOption>
        </SettingsItemBody>
    )
